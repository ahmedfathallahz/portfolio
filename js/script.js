...
function renderEducation(educationList) {
    ...
    eduContainer.innerHTML = educationList
        .map(
            edu => `
        <div class="col-lg-12">
            <div class="mdl-card exp-card mb-20" style="padding: 20px 24px;">
                <div class="exp-company">${edu.institution}</div>
                <div class="exp-role">${edu.degree} - ${edu.field}</div>
                <div class="exp-meta">
                    <span><i class="zmdi zmdi-calendar"></i> ${edu.date}</span>
                    <span><i class="zmdi zmdi-grade"></i> Grade: ${edu.grade}</span>
                </div>
            </div>
        </div>`
        )
...
function renderCertifications(certs) {
    document.getElementById("certs-cards").innerHTML = certs
        .map(
          ...
          <div class="mdl-card cert-card text-center" style="padding: 24px; height: 100%;">
